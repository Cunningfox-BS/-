...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        true_locs = torch.zeros((batch_size, n_priors, 4), dtype=torch.float)  # (N, 441, 4)
        true_classes = torch.zeros((batch_size, n_priors), dtype=torch.long)  # (N, 441)
        ...
        hardness_ranks = torch.LongTensor(range(n_priors)).unsqueeze(0).expand_as(conf_loss_neg)  # (N, 441)
